...
const Home = () => {
  ...
  return (
    <section className="relative w-full min-h-[100svh] overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[1280px] flex-col justify-center px-5 pb-10 pt-28 sm:px-8 sm:pb-14 sm:pt-32 lg:px-12">
        <motion.div
        ...
        >
          <p className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#e8eefc] backdrop-blur-sm sm:text-[12px]">
            <span className="inline-block h-1.5 w-1.5 rounded-full bg-brand-gold" aria-hidden />
          ...
          <h1 className="mt-5 font-lexend text-[2.15rem] font-bold leading-[1.12] tracking-tight text-white xs:text-[2.5rem] sm:text-[3.25rem] sm:leading-[1.08] lg:text-[4rem]">
            Get B-BBEE verified
            <span className="block text-[#c8d6f0]">with a team you can call.</span>
          </h1>
        ...
        <motion.div
          ...
          className="mt-10 grid grid-cols-1 gap-3 border-t border-white/15 pt-6 sm:mt-14 sm:grid-cols-3 sm:gap-6"
        >
          {trustItems.map((item) => (
            <div key={item.label} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 backdrop-blur-sm sm:border-0 sm:bg-transparent sm:px-0 sm:py-0 sm:backdrop-blur-0">
